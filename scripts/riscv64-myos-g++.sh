...
if [ "$LINK" -eq 1 ]; then
    exec "$BACKEND_CXX" $COMMON_FLAGS -nostdinc -nostdinc++ -isystem "$SYSROOT/include" -isystem "$SYSROOT/include/c++" -Wl,-e,_start -Wl,--gc-sections -Wl,-s "$SYSROOT/lib/cxx-crt0.o" "$@" "$SYSROOT/lib/libcxxrt.a" "$SYSROOT/lib/libc.a"
fi
