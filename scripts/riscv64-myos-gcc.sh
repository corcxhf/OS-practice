...
if [ "$LINK" -eq 1 ]; then
    exec "$BACKEND_CC" $COMMON_FLAGS $INCLUDE_FLAGS -Wl,-e,_start -Wl,--gc-sections -Wl,-s "$SYSROOT/lib/crt1.o" "$SYSROOT/lib/crti.o" "$@" "$SYSROOT/lib/libc.a" "$SYSROOT/lib/crtn.o"
fi
