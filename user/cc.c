...
{
    write_str(STDERR_FD, "usage: cc [options] file...\n");
...
void main(int argc, char *argv[], char *envp[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        {
            write_str(STDOUT_FD, "cc: MyOS system C compiler slot, currently backed by tcc\n");
            syscall(SYS_exit, 0, 0, 0);
        }
        user_argc++;
        if (append_arg(tcc_argv, &out_argc, argv[i]) < 0)
        {
            write_str(STDERR_FD, "cc: too many arguments\n");
            syscall(SYS_exit, 1, 0, 0);
        }
    }
    ...
    write_str(STDERR_FD, "cc: cannot exec /bin/tcc\n");
    syscall(SYS_exit, -1, 0, 0);
}
