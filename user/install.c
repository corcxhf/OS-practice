...
static int install_file(const char *src, const char *dst)
{
    ...
    if (in_fd < 0)
    {
        write_str(STDERR_FD, "install: cannot open ");
    ...
    if (out_fd < 0)
    {
        write_str(STDERR_FD, "install: cannot create ");
    ...
    while ((n = (int)syscall(SYS_read, in_fd, (uint64)buf, sizeof(buf))) > 0)
    {
        ...
        while (written < n)
        {
            ...
            if (m <= 0)
            {
                write_str(STDERR_FD, "install: write error ");
                write_str(STDERR_FD, dst);
                write_str(STDERR_FD, "\n");
                close_status(in_fd);
                close_status(out_fd);
    ...
    if (n < 0)
    {
        write_str(STDERR_FD, "install: read error ");
        write_str(STDERR_FD, src);
        write_str(STDERR_FD, "\n");
        close_status(in_fd);
        close_status(out_fd);
...
void main(int argc, char *argv[])
{
    if (argc != 3)
    {
        write_str(STDERR_FD, "usage: install SRC DST\n");
        syscall(SYS_exit, 1, 0, 0);
    }

    syscall(SYS_exit, install_file(argv[1], argv[2]) < 0 ? -1 : 0, 0, 0);
}
